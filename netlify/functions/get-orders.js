...
exports.handler = async () => {
  try {
    ...
    const commandes = sessions.data.map(session => ({
      ...
      client:        session.customer_details?.name || session.metadata?.prenom + ' ' + session.metadata?.nom,
      ...
      recommandations: session.metadata?.recommandations || '',
      petitesFleurs: session.metadata?.petitesFleurs || '',
